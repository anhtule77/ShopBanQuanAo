...
public class ProductService extends BaseService<Product> implements Constans{
	...
	public List<Product> findbyHot(){
		String sql = "SELECT * FROM tbl_products p WHERE category_id = 20";
		return super.executeNativeSql(sql, -1);
	}
	public List<Product> findbydress(){
		String sql = "SELECT * FROM tbl_products p WHERE category_id = 22";
		return super.executeNativeSql(sql, -1);
	}
	public List<Product> findbyshirt() {
		String sql = "SELECT * FROM tbl_products p WHERE category_id = 24";
		return super.executeNativeSql(sql, -1);
	}
	public List<Product> findbyOffice() {
		String sql = "SELECT * FROM tbl_products p WHERE category_id = 21";
		return super.executeNativeSql(sql, -1);
	}
	public List<Product> findbyClothes() {
		String sql = "SELECT * FROM tbl_products p WHERE category_id = 23 OR category_id = 24 OR category_id = 21";
		return super.executeNativeSql(sql, -1);
	}
	
	// tất cả các services cần thêm hàm search.
		public List<Product> search(ProductSearch searchModel) {
			//đóng gói tất cả các đối tượng muốn search vào trong 1 class
			// khởi tạo câu lệnh
